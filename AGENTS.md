...
src/minisweagent/trading/notify.py              交易日四个汇报点推送个人微信（WxPusher），旁路通道，未配置即跳过
...
- 微信通知是交易主路径外的旁路：凭证 `WXPUSHER_APP_TOKEN`、`WXPUSHER_UIDS`（逗号分隔）只进 `.env`，缺任一 `notify.enabled()` 即为假，宿主跳过全部取数与排版；推送失败只回显加日志，绝不 raise 打死交易日。汇报内容直接用流水线里已产生的模型输出（候选 reason、执行 submission、复盘报告），不额外发模型请求。
- 永远不要序列化或记录 `DS_KEY`。
