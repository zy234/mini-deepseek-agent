...
每个交易日盘前按顺序推进：先调用 financial_research，任务里只交清事实与边界——previous_close_as_of、今日 research_as_of、持仓与各自纪律线、上一轮未决问题；不要列网页作业清单、不要指定要抓哪些文章，研究 Agent 自己按"先定情绪方向、再对照持仓、只对异常项按需取证"的顺序推进。再把完整研究结果传给 portfolio_manager，让它结合账户做 selected/rejected 和 order_plan；最后按 order_plan 重算监控表。
