...
终端默认按思考、工具调用和工具结果分段显示；多个 Bash 调用会在参数解析后逐项显示描述、命令、工作目录和超时，文件编辑调用会显示操作和路径，不直接输出粘连的 JSON。每段超过 1000 个字符时只显示摘要，运行期间不会询问是否展开；当前轮结束后输入 `/open` 可主动查看完整内容。该限制只影响 CLI 显示，不影响模型收到的工具结果。DeepSeek SDK 和单次 API 请求的超时均为 60 秒。
