...
这是一个从 mini-SWE-agent 精简出来的单模型 Agent 框架：模型固定为 `deepseek-v4-flash`，提供 host-owned 的 `bash`、`str_replace_editor` 和 `web_search` 三个工具。网页搜索由仓库自带的零 Key 多引擎实现提供。
...
`DS_KEY` 只用于调用 DeepSeek 主模型；`web_search` 不需要该 Key，也不会把 Key 传给搜索引擎。
...
终端默认按思考、工具调用和工具结果分段显示；Bash、文件编辑和网页搜索调用会显示可读的参数，不直接输出粘连的 JSON。每段超过 1000 个字符时只显示摘要，运行期间不会询问是否展开；当前轮结束后输入 `/open` 可主动查看完整内容。该限制只影响 CLI 显示，不影响模型收到的工具结果。DeepSeek SDK 和单次 API 请求的超时均为 60 秒。
...
CLI
 └── DefaultAgent
      ├── DeepSeekModel
      │    └── 工具协议：bash、str_replace_editor、web_search
      ├── LocalEnvironment
      │    ├── subprocess 执行 Bash
      │    ├── 工作区内原子文件编辑
      │    └── 仓库自带的零 Key 多引擎网页搜索
...
- `src/minisweagent/models/deepseek_model.py`：通过 OpenAI SDK 请求 DeepSeek，解析三类工具调用。
...
- `src/minisweagent/models/utils/actions_toolcall.py`：定义 Bash、编辑器和网页搜索工具及 tool 结果消息。
